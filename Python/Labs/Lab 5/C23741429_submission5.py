...
#
# Magic methods implementation within the mystery game
...
class Game:
    ...
    def __init__(self):
        ...
        self.characters_interacted = 0  # Track how many characters the player interacts with
        self.clues_found = 0  # Track the number of clues found
        ...
        self.witness2 = Witness("Ms. Harris", "I heard footsteps", "Unknown figure")
    ...
    def interact_with_characters(self):
        ...
        if choice == 's':
            ...
            print(self.suspect.interact())
            self.suspect.perform_action()
            self.characters_interacted += 1  # Track interactions
            
            print(self.witness.interact())
            self.witness.perform_action()
            self.characters_interacted += 1  # Track interactions
            
            # Combine two witnesses using the + operator
            combined_witness = self.witness + self.witness2
            combined_witness.share_observation()
    ...
    def examine_clues(self):
        ...
        if not self.crime_scene.investigated:
            ...
            self.clues_found += 1  # Track clue discovery
    ...
    def __score__(self):
        """Calculates and returns the player's score based on interactions and clues."""
        score = (self.characters_interacted * 10) + (self.clues_found * 20)
...
class Character(ABC):
    ...
    # Basic Magic Method Implementations
    def __str__(self):
        """String representation of the derived class and the character's name."""
        return f"{self.__class__.__name__}({self._name})"
    
    def __lt__(self, char):
        """Less than operator overload based on character name."""
        return self._name < char._name
    
    def __eq__(self, char):
        """Equality operator overload based on character name."""
        return self._name == char._name
...
class Suspect(Character):
    ...
    def __repr__(self):
        """Formal representation of the suspect showing their name, dialogue and alibi.
        
        Returns:
            The new representation of the object
        """
        return f"<Suspect: {self._name}, Dialogue: {self._dialogue}, Alibi: {self.alibi}>"
...
class Witness(Character):
    ...
    def share_observation(self):
        """Shares the witness's observation."""
        print(f"{self._name} witnessed: {self.witness}")

    # Magic Method overloading the + operator for observations
    def __add__(self, char):
        """Overloads the + operator to combine the observations of two Witness objects.
        Returns:
            Instance of Witness
        """
        new_statement = self.witness + " " + char.witness
        return Witness(self._name, new_statement, self.suspect)
...
if __name__ == "__main__":
    ...
    # suspect1 = Suspect("Mr. Smith", "in the library", "Confirmed by the butler.")
    # suspect2 = Suspect("Mr. Collins", "in the kitchen", "Confirmed by the chef")
    # witness1 = Witness("Ms. Parker", "I saw someone near the window", "Suspicious figure")
    # witness2 = Witness("Ms. Harris", "I heard footsteps", "Unknown figure")

    # print(suspect1.interact())
    # print(suspect2.interact())
    # print(witness1.interact())
    # print(witness2.interact())
    ...
    # print(f"Comparison of the two suspects names: {suspect1 == suspect2}")

    # print(f"Is suspect1's name less than suspect 2?: {suspect1 < suspect2}")

    # print(repr(suspect1))

    # new_witness = witness1 + witness2
    # new_witness.share_observation()
